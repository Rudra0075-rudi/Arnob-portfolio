...
function deleteLocalUploadIfExists(url) {
  ...
  // ensure basename only (no nested dirs)
...
router.post('/', requireAdmin, upload.single('coverImage'), async (req, res) => {
  console.log('Blog POST body:', req.body);
  try {
    ...
    res.status(400).json({ message: 'Failed to create blog', error: String(e) });
  }
});
...
router.put('/:id', requireAdmin, upload.single('coverImage'), async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'Failed to update blog', error: String(e) });
  }
});
...
router.delete('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'Failed to delete blog', error: String(e) });
  }
});
